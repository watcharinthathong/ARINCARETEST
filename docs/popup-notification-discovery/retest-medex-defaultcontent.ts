/**
 * ทดสอบด้วยเนื้อหา default ที่ฟอร์มเติมมาให้เอง (ไม่แตะ Raw HTML, ไม่กด Quick Template)
 * เลียนแบบว่า "cc" (แคมเปญที่ทำงานได้จริง) น่าจะสร้างแบบไหน
 */
...
(async () => {
  ...
  const code = `QA_MEDEX_DEFCONTENT_${Date.now()}`;
  await adminPage.goto(`${ADMIN_BASE}/popup-notifications/create`, { waitUntil: 'domcontentloaded' });
  await adminPage.waitForTimeout(800);
  await adminPage.getByPlaceholder('เช่น แจ้งปรับปรุงระบบประจำเดือน, ข่าวสารประชาสัมพันธ์').fill('QA MedEx DefaultContent');
  ...
  // ⚠️ ไม่แตะ Editor เลย ไม่กด Quick Template ไม่สลับ Raw HTML — ปล่อยเนื้อหา default ไว้ตามที่ฟอร์มให้มา
  const editorText = await adminPage.locator('.custom-wysiwyg, [contenteditable="true"]').first().innerText().catch(() => '(หา editor ไม่เจอ)');
  console.log('เนื้อหา default ในฟอร์ม:', editorText.slice(0, 100));
  ...
  console.log('สร้างสำเร็จ:', code, 'URL:', adminPage.url());
  ...
  console.log('Popup แสดงไหม (เนื้อหา default ไม่แก้ไข):', visible);
  await clientPage.screenshot({ path: path.join(__dirname, 'medex-defaultcontent-result.png'), fullPage: true });
